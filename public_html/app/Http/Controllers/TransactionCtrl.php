...
class TransactionCtrl extends Controller
{
    ...
    {
        $data_transactions = Transactions::with(['pasien', 'dokter', 'transaction_tindak', 'transaction_tindak.tindakan'])->orderBy('id', 'DESC')->get();
    ...
    public function preview(Request $request)
    {
        ...
        $data_tindakan = Tindakan::all(["id", "nama_tindakan", "total_harga","biaya_lain","keterangan"]);
        ...
        if ($request->tindakans) {
            ...
            foreach ($tindakans as $tindak) {
                TransactionTindakan::create([
                    ...
                    "biayalain" => intval($tindak["biayalainInput"]),
                    "keteranganlain" => $tindak["keteranganlainInput"],
    ...
    public function createTindakan(Request $request)
    {
        ...
        $transaction = TransactionTindakan::create([
            ...
            "biaya" => $request->biaya,
            "biayalain" => $request->biaya_lain,
            "keteranganlain" => $request->keterangan_lain,
    ...
    public function editTindakan(Request $request)
    {
        ...
        $tindakan->update([
            "biayalain" => intval($request->biaya_lain),
            "keteranganlain" => $request->keterangan_lain,
            "quantity" => $request->quantity,
            "discount" => $request->diskon,
            "subtotal" => $subtotal,
    ...
    public function transactionByDokter(Request $request)
    {
        ...
        // $data_transactions = User::with(['transaksi' => function ($query) {
        //     return $query->whereBetween('created_at', [date("Y-m-d", strtotime("-7 days")), date("Y-m-d", strtotime("+1 day"))]);
        // }])->get();

        // if ($request->has('startDate') && $request->has('endDate')) {
        //     $startDate = $request->query('startDate');
        //     $endDate = date($request->query('endDate'), strtotime("+1 day"));
        //     $data_transactions = User::with(['transaksi' => function ($query) use ($startDate, $endDate) {
    ...
    {
        $data_transactions = Tindakan::with(['transaction_tindak' => function ($query) {
            return $query->whereBetween('created_at', [date("Y-m-d", strtotime("-7 days")), date("Y-m-d", strtotime("+1 day"))]);
        }])->get();

        if ($request->has('startDate') && $request->has('endDate')) {
            $startDate = date($request->query('startDate'));
            $endDate = date($request->query('endDate'), strtotime("+1 day"));
            $data_transactions = Tindakan::with(['transaction_tindak' => function ($query) use ($startDate, $endDate) {
                return $query->whereBetween('created_at', [$startDate, $endDate]);
            }])->get();
