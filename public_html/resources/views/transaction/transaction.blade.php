        ...
        <tbody>
            @foreach ($data_transactions as $transaction)
                <tr>
                    ...
                    <td>{{ number_format($transaction->transaction_tindak->sum('subtotal'), 0, ',', '.') }}</td>
                    <td>
                        ...
                        <a href="#" id="notaBtn{{ $transaction->id }}" class="btn btn-sm btn-primary" data-nomor_transaksi="{{ $transaction->nomor_transaksi }}"
                            ...
                            data-telepon_pasien="{{ $transaction->pasien ? $transaction->pasien->telepon : '' }}" data-keterangan="{{ $transaction->keterangan }}" data-data_tindakans="{{ $transaction->transaction_tindak }}"
