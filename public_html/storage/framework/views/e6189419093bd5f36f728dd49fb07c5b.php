...
<script>
    window.onload = function() {
        ...
        function processDataTindakan(data) {
            ...
            // Loop through each tindakan in the data
            for (let i = 0; i < data.length; i++) {
                let tindakan = data[i];

                // Get the nama_tindakan and count the number of transaction_tindak entries
                let nama_tindakan = tindakan.nama_tindakan;
                let count = tindakan.transaction_tindak.length;

                // Add the count to the existing count for the nama_tindakan, or initialize it to 0
                tindakanMap[nama_tindakan] = (tindakanMap[nama_tindakan] || 0) + count;
            }

            // Convert the tindakanMap into an array of data points
            for (let nama_tindakan in tindakanMap) {
                let count = tindakanMap[nama_tindakan];
                let dataPoint = {
                    label: nama_tindakan,
                    name: nama_tindakan,
                    y: count
                };
                chartDataTindakan.push(dataPoint);
            }
    ...
    document.getElementById('generateChartTindakan').addEventListener('click', function() {
        ...
        fetch(`/transaction/chart/tindakan?startDate=${dateMin}&endDate=${dateMax}`) // Replace with your API endpoint
            .then(response => response.json())
            .then(data => {
                processDataTindakan(data);
            })
...
<?php echo $__env->make('templating.template_with_sidebar', ['isActiveInfografis' => 'active'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u726706882/domains/sistem.oklusif.com/public_html/resources/views/infografis/infografis.blade.php ENDPATH**/ ?>